...
Generalized Linear Models (GLMs) form the foundation of insurance predictive modeling. Traditional linear regression has key limitations - it assumes the response variable can take any real value and maintains constant variance. However, insurance data often violates these assumptions. The GLM framework extends linear regression by allowing:
...
Understanding model assumptions is crucial for reliable predictions. Logistic regression has several key assumptions:
...
2. Technical Approaches:
  - Basic transformations (log, box-cox, polynomial)
...
We can use several techniques to handle imbalanced datasets and 
#### 1. Class Weights
...
In insurance modeling, while GLMs provide interpretability and regulatory acceptance, modern machine learning algorithms offer significant advantages for complex risk assessment and fraud detection.
